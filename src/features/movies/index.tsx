...
const Home = () => {
  ...
  return (
    <div className="flex flex-col m-3">
      <label className="text-xl font-semibold mt-5 mb-5 text-center mx-auto">Movies List</label>
      {!loading ? (
      ...
      <div className="flex flex-row justify-center gap-5 p-5">
        <button
          ...
          className= "bg-slate-700 text-white p-2 rounded-sm"
          disabled={Number(page) <= 1}
        >
          Back
        </button>
        <button onClick={handleNextPage} className= "bg-slate-700 text-white p-2 rounded-sm">
          Next
        </button>
      </div>
      {selectedMovie && (
